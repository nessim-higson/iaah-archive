...
var Canvas2Image = function () {
  ...
  function saveFile (strData, type, filename) {
    ...
    link.href = strData;
    link.download = filename+'.jpg'; // temp, fix with mime type
  ...
  var saveAsImage = function (canvas, width, height, type, filename) {
    if ($support.canvas && $support.dataURL) {
      ...
      if (/bmp/.test(type)) {
        ...
        saveFile(makeURI(strData, downloadMime), type, filename);
      } else {
        var strData = getDataURL(canvas, type, width, height);
        saveFile(strData.replace(type, downloadMime), type, filename);
      }
    }
  };
